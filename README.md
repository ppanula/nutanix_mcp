...
Ask GitHub Copilot questions about your Nutanix environment in plain English — VMs, hosts, storage, networking, alerts, tasks, protection domains, resource utilization, migration plans, and more — from VS Code or GitHub Copilot CLI.
...
- **or** [GitHub Copilot CLI](https://docs.github.com/copilot/how-tos/use-copilot-agents/use-copilot-cli)
- GitHub Copilot access (VS Code extensions or Copilot CLI login)

## Quick start

```bash
# 1. Install
uv tool install nutanix-mcp
...
If you prefer one-off execution without installing the tool globally:

```bash
uvx nutanix-mcp configure
```
...
- Ask which target(s) to configure (VS Code, Copilot CLI, or both)
- Write MCP config files with no credentials in them
...
Open Copilot Chat, switch to **Agent mode**, and start asking questions.
...
From interactive mode, run `/mcp show` to verify `nutanix-mcp` is available, then start asking questions.
...
Generated by `nutanix-mcp configure` when you choose the Copilot CLI target.  
If `COPILOT_HOME` is set, the file is written to `$COPILOT_HOME/mcp-config.json`.
...
```json
{
  "mcpServers": {
    "nutanix-mcp": {
      ...
      "args": [],
      "env": {
        "NUTANIX_MCP_INVENTORY": "/absolute/path/to/inventory.yaml"
      },
      "tools": ["*"]
    }
  }
...
In VS Code (Copilot Chat Agent mode) or GitHub Copilot CLI, ask in plain English:
...
- Transport is **stdio** — the server runs locally and is managed by VS Code or Copilot CLI
